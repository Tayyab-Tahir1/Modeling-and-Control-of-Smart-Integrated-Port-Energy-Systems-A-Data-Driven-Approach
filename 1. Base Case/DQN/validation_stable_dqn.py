...
ENERGY_PER_KG_H2 = 32.0       # kWh per kg H2 (UPDATED)
...
# (It will be converted to kg/kWh in the code.)
EMISSION_FACTOR = 0.5
...
def compute_max_power(df):
    # Use maximum among Load, PV, battery power (BATTERY_CAPACITY * C_RATE), and a reference value.
...
def process_action_new(action, load, pv, tou_tariff, fit, h2_tariff, soc, h2_storage, emission_factor_used):
    ...
    # Emissions using the provided emission factor (kg/kWh)
...
# =========================
# Main Validation Script for DQN
# =========================
def main():
    ...
    print("\nRunning validation with Stable-Baselines3 DQN model (expanded action space)...")
    for index, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        # Determine emission factor: if the row has 'Emission_factor', convert from gram/kWh to kg/kWh.
    ...
    output_csv = "results_stable_updated.csv"
    ...
    plt.tight_layout()
    plt.savefig("energy_management_results_stable_updated.png")
    plt.close()
    print("Plots saved to 'energy_management_results_stable_updated.png'.")
