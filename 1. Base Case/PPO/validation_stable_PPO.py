...
# Default emission factor (if no column is provided): in gram/kWh.
DEFAULT_EMISSION_FACTOR = 0.5  
# (Later, in validation, we convert from gram to kg by dividing by 1000.)
...
def process_action_new(action, load, pv, tou_tariff, fit, h2_tariff, soc, h2_storage, emission_factor_used=DEFAULT_EMISSION_FACTOR/1000.0):
    ...
    # Calculate emissions using the passed emission_factor (in kg/kWh)
...
def main():
    ...
    # Add new columns from original dataset along with MPC outputs.
    allocation_columns = [
        'pv_to_load', 'pv_to_battery', 'pv_to_grid',
        'battery_to_load', 'grid_to_load', 'grid_to_battery',
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        # Determine emission factor: if the row has 'Emission_factor', use it (convert from grams to kg), else use default.
        ...
        soc, h2_storage, allocations, purchase, sell, bill, emissions, reward = process_action_new(
            raw_action, load, pv, tou_tariff, fit, h2_tariff, soc, h2_storage, emission_factor_used=emission_factor_used
        )
