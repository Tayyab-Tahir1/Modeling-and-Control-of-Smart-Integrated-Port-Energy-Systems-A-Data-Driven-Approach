...
def get_expert_action(flows):
    ...
    The function selects the action whose corresponding flow value is the largest and above a small threshold.
    """
    threshold = 1e-3
    action_values = {
        1: flows.get('battery_to_load', 0),
        2: flows.get('pv_to_battery', 0),
        3: flows.get('hydrogen_produced', 0) * ENERGY_PER_KG_H2,  # Convert kg to kWh for consistency.
        4: flows.get('h2_to_load', 0),
        5: flows.get('grid_to_battery', 0),
        6: flows.get('h2_to_battery', 0),
        7: flows.get('h2_to_load_purchased', 0)
    }
    max_action = 0  # Default action is 0 ("do nothing")
    max_val = 0.0
...
# ------------------------------------------------
# 2) Sequence Dataset Builder (Modified)
# ------------------------------------------------
def build_finetuning_sequences(df, seq_len=24):
    ...
    Modification:
      - Each sequence is simulated independently starting with the initial state.
      - This removes inter-sequence state drift and ensures that each sequence represents an independent MPC rollout.
    """
    ...
    # Adjust the loop to ensure each sequence has room for the forecast horizon.
    for i in tqdm(range(N - seq_len - HORIZON + 1), desc="Building expert sequences"):
        # Reinitialize state for every sequence independently.
        ...
        for t in range(i, i + seq_len):
            ...
            # Ensure there is sufficient forecast horizon ahead.
            if t + HORIZON > N:
...
def train_lstm(model, sequences, initial_epochs, batch_size, lr, patience=20, max_epochs=5000):
    ...
    Training runs for at least `initial_epochs` epochs. If the average loss does not improve
...
def main():
    ...
    wandb.init(project="MPC_Imitation_LSTM", config=vars(args))
