...
# =========================
# Modified Process Dataset Function for DQN Validation (Updated for PPO)
# =========================
def process_dataset(country_code, dataset_path, model):
    # Read dataset with all original columns preserved.
    ...
    # Create a list to hold merged dictionaries.
    results = []
    
    soc = BATTERY_CAPACITY * 0.5
    h2_storage = 0.0
    
    print(f"\nRunning validation for {country_code} with the PPO model...")
    for index, row in tqdm(df_original.iterrows(), total=df_original.shape[0]):
        ...
        # Use default values for Day and Hour if not present.
        day = row.get('Day', 0)
        hour = row.get('Hour', 0)
...
# =========================
# Main Validation Script for Multiple Datasets (PPO)
# =========================
def main():
    ...
    for country_code, dataset_file in datasets.items():
        ...
        output_csv = os.path.join(country_folder, f"results_ppo_stable_updated_{country_code}.csv")
        df_result.to_csv(output_csv, index=False)
        print(f"Results saved to {output_csv}")

        # Log per-hour metrics to wandb using the specified order.
        wandb.init(project="Thesis_PPO_location", name=f"{country_code}_validation", reinit=True)
        for i, row in df_result.iterrows():
            # Log keys in this order:
            log_data = {
                "Bill": row["Bill"],
                "Sell": row["Sell"],
