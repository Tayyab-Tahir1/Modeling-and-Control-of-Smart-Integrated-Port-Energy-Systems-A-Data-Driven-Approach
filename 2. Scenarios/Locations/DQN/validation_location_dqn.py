...
# Global reference for maximum power
self_max_power = None

def compute_max_power(df):
    # Use maximum among Load, PV, battery power, and fuel cell capacity.
...
def get_feasible_actions_new(load, pv, tou_tariff, h2_tariff, soc, h2_storage):
    ...
    Expanded action space (0–7):
      ...
      7: Purchase hydrogen from the grid to meet load
    """
    feasible = [0]
    if soc > SOC_MIN + 1e-5:
        feasible.append(1)
...
def process_action_new(action, load, pv, tou_tariff, fit, h2_tariff, soc, h2_storage, emission_factor_used):
    ...
    H2_purchase_cost = 0  # cost if hydrogen is purchased
...
# =========================
# Modified Process Dataset Function for DQN Validation
# =========================
def process_dataset(country_code, dataset_path, model):
    # Read dataset (all original columns are preserved).
    ...
    # Define computed columns to add.
    computed_columns = [
        'pv_to_load', 'pv_to_battery', 'pv_to_grid',
        'battery_to_load', 'grid_to_load', 'grid_to_battery',
    ...
    # Create a list to hold merged dictionaries for each row.
    results = []
    
    soc = BATTERY_CAPACITY * 0.5
    h2_storage = 0.0
    
    print(f"\nRunning validation for {country_code} with the DQN model...")
    ...
    for index, row in tqdm(df_original.iterrows(), total=df_original.shape[0]):
        ...
        # 'Day' and 'Hour' and any other original columns are in row.
        ...
        state = np.array([
            ...
            row.get('Day', 0) / 6.0,      # use default if missing
            row.get('Hour', 0) / 23.0       # use default if missing
        ], dtype=np.float32)
        ...
        soc, h2_storage, allocations, purchase, sell, bill, emissions, reward = process_action_new(
            raw_action, load, pv, tou_tariff, fit, h2_tariff, soc, h2_storage, emission_factor_used
        )

        # Create a merged dictionary containing all original columns and computed outputs.
        res_dict = row.to_dict()    # keeps all original columns
        res_dict.update({
    ...
    # Create a DataFrame with all original and computed columns.
    df_out = pd.DataFrame(results)
    return df_out

# =========================
# Main Validation Script for Multiple Datasets (DQN)
# =========================
def main():
    ...
    for country_code, dataset_file in datasets.items():
        ...
        df_result = process_dataset(country_code, dataset_file, model)
        
        # Create a folder for the country inside "Results" and save CSV.
        country_folder = os.path.join(results_dir, country_code)
        if not os.path.exists(country_folder):
            os.makedirs(country_folder)
        output_csv = os.path.join(country_folder, f"results_stable_updated_{country_code}.csv")
        df_result.to_csv(output_csv, index=False)
        print(f"Results saved to {output_csv}")

        # Log per-hour metrics to wandb using the final log format with keys in the specified order.
        wandb.init(project="Thesis_DQN_location", name=f"{country_code}_validation", reinit=True)
